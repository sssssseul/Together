...
app.get('/api/history', async (req, res) => {
  try {
    ...
    const countRes = await pool.query('SELECT COUNT(*) FROM couple_draws WHERE date <> $1', [today]);
    const total = parseInt(countRes.rows[0].count, 10);
    const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));

    const rowsRes = await pool.query(
      'SELECT date, card_index, review FROM couple_draws WHERE date <> $1 ORDER BY date DESC LIMIT $2 OFFSET $3',
      [today, PAGE_SIZE, (page - 1) * PAGE_SIZE]
    );
